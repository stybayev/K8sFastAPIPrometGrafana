...
@pytest.mark.anyio
async def test_yandex_callback_success(
    auth_async_client: AsyncClient, mock_db_session, request_headers
):
    """
    ...
    with (
        patch.object(
            OAuthService, "get_token", return_value={"access_token": "valid_token"}
        ),
        patch.object(
            OAuthService,
            "get_user_info",
        ...
        patch.object(
            OAuthService, "get_or_create_user", return_value={"id": "user_id"}
        ),
        patch.object(
            OAuthService,
        ...
        response = await auth_async_client.get(
            f"/yandex/callback?code={code}", headers=request_headers
        )
...
@pytest.mark.anyio
async def test_yandex_callback_missing_code(
    auth_async_client: AsyncClient, request_headers
):
    """
...
@pytest.mark.anyio
async def test_unlink_yandex_account_success(
    auth_async_client: AsyncClient, mock_db_session, request_headers
):
    """
    ...
    with (
        ...
        response = await auth_async_client.delete(
            "/yandex/unlink", headers=request_headers
        )
...
@pytest.mark.anyio
async def test_unlink_yandex_account_not_found(
    auth_async_client: AsyncClient, mock_db_session, request_headers
):
    """
